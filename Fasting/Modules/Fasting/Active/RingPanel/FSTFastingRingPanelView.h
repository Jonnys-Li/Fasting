...
/// 圆环的视觉表现态。三态实际上 ring 主色都是 `fst_eatingTimeGreen`（绿），差异在装饰：
/// 火焰沿弧 / completion icon / endTail 末端渐变。颜色仅在两 panel 之间区分，本 panel 内不变。
/// 写入方：FSTActiveFastingDisplayState 工厂依据 timing.overtime / targetReached 推导（overtime 优先于 complete）。
/// 读取方：FSTFastingRingPanelView 内部 — 切换 endTail 渐变可见性、中央文本字号、子视图可见性（modeButton / completionIcon / overtimeLabel）。
typedef NS_ENUM(NSInteger, FSTRingPresentationState) {
    FSTRingPresentationActive,    ///< 进行中（未达标）。绿环 + 中央 HH:MM:SS + 火焰沿弧 + modeButton 可见。
    FSTRingPresentationComplete,  ///< 已达标但未超时。绿环 + completion icon + 大字 "100%"（ring 视觉与 Active 一致）。
    FSTRingPresentationOvertime,  ///< 已超时。绿环 + endTail 末端渐变高亮 + 中央 "+HH:MM:SS" + 副文本。优先级高于 Complete。
};
