...
//    具体走哪条由 -refreshUI 内部判定（详见 .m）。
//  - 输入：FSTSessionDidChangeNotification（外部改 plan / 完成断食等）+ refreshTimer（每秒推动倒计时）。
...
@interface FSTDailyPlanViewController : FSTBaseViewController
