...
#pragma mark - FSTRingStyle 一键样式

@implementation FSTRingProgressView (Style)

- (void)fst_applyStyle:(FSTRingStyle)style {
    UIColor *progressColor = [UIColor fst_eatingTimeGreen];
    UIColor *arrowTint     = [UIColor whiteColor];
    BOOL showsEndTail      = NO;

    switch (style) {
        case FSTRingStyleActiveFasting:
        case FSTRingStyleCompleteFasting:
            break;  // 默认值即对
        case FSTRingStyleOvertimeFasting:
            showsEndTail = YES;
            break;
        case FSTRingStyleEatingWindow:
        case FSTRingStyleScheduledCountdown:
            progressColor = [UIColor fst_progressCream];
            arrowTint     = [UIColor fst_amber];
            break;
        case FSTRingStyleReadyToStartFasting:
            progressColor = [UIColor fst_ringReadyOrange];
            arrowTint     = [UIColor whiteColor];
            break;
    }

    self.progressColor      = progressColor;
    self.endTailColor       = [UIColor fst_progressCream];  // 6 态共用末端配色
    self.endTailFraction    = 0.10;
    self.showsEndTail       = showsEndTail;
    self.arrowHeadTintColor = arrowTint;
}
